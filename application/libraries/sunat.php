<?php
	...
	class Sunat{
		...
		function getNumRand()
		{
			$url="http://www.sunat.gob.pe/cl-ti-itmrconsruc/captcha?accion=random";//"http://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/captcha?accion=random";
		...
		function getDataRUC( $ruc )
		{
			...
			if($ruc != "" && $numRand!=false)
			{
				...
				$url = "http://www.sunat.gob.pe/cl-ti-itmrconsruc/jcrS03Alias";//"http://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/jcrS00Alias";
				...
				if(isset($matches[0]))
				{
					$RS = utf8_encode(str_replace($ruc.' - ','', ($matches[0][1])));
					$rtn = array("ruc"=>$ruc,"razon_social"=>trim($RS));
				}

				//echo $patron." - ";print_r($matches);print_r($rtn);exit();
